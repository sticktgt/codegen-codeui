...
Если в шаге есть `error_type`, `error_message` или `exception_class`, UI показывает ошибку прямо в строке этого шага. Длинное сообщение об ошибке отображается компактно и раскрывается через отдельный блок с полным текстом. Для шагов без ошибки отображение остается обычным.
