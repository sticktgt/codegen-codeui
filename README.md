...
`codeui` не хранит проекты как отдельную бизнес-сущность. Список проектов читается из `codecollector`. В общем `ui_state.json` сохраняются выбранный проект и путь к файлу требований. Выбранное требование, выбранный CR и выбранный запуск хранятся локально в `sessionStorage` вкладки браузера.
...
В выпадающем списке проектов показывается название проекта. При выборе проекта значение сохраняется в общем `ui_state.json` и становится общим значением по умолчанию для открытого UI.
...
Общее состояние хранится в `ui_state.json`. В нем используются:

- `selected_project_id` — активный проект по умолчанию;
- `requirements_file_path` — путь к текущему файлу требований.

Локальное состояние вкладки браузера хранится в `sessionStorage`:

- выбранное требование;
- выбранный CR;
- выбранный запуск.

Такой режим позволяет нескольким пользователям работать с разными требованиями, CR и запусками одновременно без аутентификации. Общими остаются список проектов, путь к файлу требований, сами CR и run artifacts.
...
Требования в UI не редактируются. Путь к файлу требований хранится в общем `ui_state.json`. Выбранное требование хранится локально в текущей вкладке браузера и не перетирает выбор других пользователей. При создании CR в него сохраняется snapshot выбранных требований. Это позволяет отображать CR даже в случае недоступности исходного файла требований.
...
Список CR фильтруется по активному проекту `codecollector`. Если проект не выбран, показываются все CR. Выбранный CR хранится в `sessionStorage` текущей вкладки браузера, поэтому несколько пользователей могут одновременно открывать разные CR без перезаписи общего `ui_state.json`.
...
На вкладке `3. Запуски` можно открыть конкретный запуск. Выбранный запуск хранится в `sessionStorage` текущей вкладки браузера. Если запуск готов к ручному применению и содержит workspace id, в панели `Результат запуска` доступна кнопка `Применить`. Она применяет workspace именно выбранного запуска, а не обязательно последнего запуска CR. Если запуск связан с CR, после успешного применения CR получает финальный статус `applied`, а `applied_run_id` указывает на выбранный запуск.
...
`data/ui_state.json` хранит выбранный проект и путь к файлу требований. Выбранные требования, выбранный CR и выбранный запуск хранятся в `sessionStorage` вкладки браузера.
...
- `codeui/services/ui_state_service.py` хранит общий state UI: выбранный проект и путь к файлу требований.
- `codeui/services/project_lock_service.py` управляет файловыми блокировками операций проекта.
...
## Блокировки операций проекта

Для операций, которые меняют состояние основного проекта или его индекс, используется файловая блокировка по `project_id`. Блокировка применяется к:

- применению workspace последнего результата CR;
- применению workspace выбранного запуска;
- обновлению индекса проекта;
- удалению проекта.

Если проект уже занят другой такой операцией, API возвращает controlled error `PROJECT_OPERATION_BUSY`, а UI показывает сообщение, что проект сейчас занят. Генерация pipeline не блокируется этой блокировкой, потому что она работает со staging workspace и может выполняться параллельно.

Каталог блокировок задается в `config.yaml`:

```yaml
project_locks:
  storage_dir: "data/locks/projects"
